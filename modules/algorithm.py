...
class Algorithm:
    ...
    def _init(self):
        ...
        if os.path.exists(self.config.cached_vecs_path):
            ...
            self.cached_vecs = {}
        print('Load data done, candidate number: {}, text line number: {}, cached vocab vectors: {}'.format(len(self.candidates), len(self.text), len(self.cached_vecs)))
    ...
    def _get_vector(self, batch):
        ...
        input = [torch.tensor(c+[0]*(max_len-len(c)), dtype=torch.long) for c in input]
        input = torch.stack(input)
        if not self.config.cpu:
            input = input.cuda()
        with torch.no_grad():
            h, _ = self.bert(input, attention_mask=(input>0), output_all_encoded_layers=False)
            h = h.detach()
    ...
    def get_vector(self):
        ...
        print('Load vec done, concepts number: {}, vecs shape: {}'.format(len(self.concepts), self.vecs.shape))
