...
class Crawler:
    ...
    def get_snippet(self, concept):
        conn = sqlite3.connect(self.config.db)
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM {} WHERE concept=?'.format(self.config.snippet_source), (concept, ))
        res = cursor.fetchall()
        if not res:
            snippet = self.crawl_snippet(concept)
            print('get snippet {} from source {}'.format(concept, self.config.snippet_source))
            cursor.execute('INSERT INTO {} (concept, snippet) VALUES (?,?)'.format(self.config.snippet_source), (concept, snippet, ))
            conn.commit()
        else:
            snippet = res[0][1]
        conn.close()
